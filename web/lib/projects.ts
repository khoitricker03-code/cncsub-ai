...
function sanitizeProjectName(name: string): string {
  ...
  return cleaned.slice(0, 120) || "Dự án chưa đặt tên";
}
...
export async function createProject(name?: string): Promise<ProjectRecord> {
  ...
  await Promise.all([
    fs.mkdir(path.join(root, "media"), { recursive: true }),
    fs.mkdir(path.join(root, "transcript"), { recursive: true }),
    fs.mkdir(path.join(root, "render"), { recursive: true }),
    fs.mkdir(path.join(root, "history"), { recursive: true }),
  ]);
...
export async function getProject(
  ...
  try {
    const content = await fs.readFile(projectFile(projectId), "utf8");
